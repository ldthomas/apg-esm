...
export default class SabnfParser {
  ...
  generateSource(chars, lines, rules, udts, typescript) {
    ...
    source += 'interface GrammarOpcodeBase {\n';
    source += '  type: number;\n';
    source += '  gl: number;\n';
    source += '  go: number;\n';
    source += '}\n';
    source += '\n';
    source += 'type GrammarOpcode =\n';
    source += '  | (GrammarOpcodeBase & { children: number[] })\n';
    source += '  | (GrammarOpcodeBase & { index: number })\n';
    source += '  | (GrammarOpcodeBase & { min: number; max: number })\n';
    source += '  | (GrammarOpcodeBase & { string: number[] });\n';
    source += '\n';
    ...
    source += '}\n';
    source += '\n';
    source += 'export default function grammar(this: {\n';
