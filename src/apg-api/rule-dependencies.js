...
let state = null; /* keep a module-level reference to the state for the show functions */

/* scan the opcodes of the indexed rule and discover which rules it references and which rule refer back to it */
function scan(ruleCount, ruleDeps, index, isScanned) {
  let i = 0;
  let j = 0;
...
function ruleDependencies(stateArg) {
...
function showRuleDependencies(order = 'index') {
